...
class DebateManager:
    ...
    def run(
        ...
        round_updates: dict[str, dict[str, Any]] | None = None,
    ) -> DebateSummary:
        ...
        debated_memos = self._apply_round_updates(memos, round_updates)
        recomputed = self.calculator.calculate(debated_memos)
        retained = recomputed.hard_dissent_present
        if recomputed.consensus_score < 0.7:
            stop = "low_consensus_blocked"
            next_stage = "blocked"
            execution_blocked = True
        elif recomputed.action_conviction < 0.65:
            stop = "low_action_conviction_blocked"
            next_stage = "observe_or_reopen"
            execution_blocked = True
        elif retained:
            stop = "hard_dissent_risk_handoff"
            next_stage = "enter_s4_with_s5_hard_dissent_review"
        ...
        return self._summary(
            ...
            execution_blocked,
            recomputed,
            retained_hard_dissent=retained,
    ...
    def _summary(
        ...
        command_statuses = {
            "ask_question": "accepted",
            "request_view_update": "accepted",
            "request_evidence": "expired",
            "request_agent_run": "rejected",
        }
        commands = [
            {
            ...
            for command_type, status in command_statuses.items()
        ]
        view_updates = [
            {
                "view_update_id": new_id("view-update"),
                "producer_agent_id": memo.analyst_id,
                "changed_fields": sorted((round_updates or {}).get(memo.analyst_id, {"comment": True})),
                "evidence_refs": memo.supporting_evidence_refs,
            }
            for memo in memos
        ] if rounds else []
        ...
        return DebateSummary(
            ...
            issues=["hard_dissent_evidence_gap"] if result.hard_dissent_present else ["consensus_alignment"],
    ...
    def _apply_round_updates(
        memos: list[AnalystMemo],
        round_updates: dict[str, dict[str, Any]] | None,
    ) -> list[AnalystMemo]:
        if not round_updates:
            return memos
        updated_memos: list[AnalystMemo] = []
        for memo in memos:
            updates = round_updates.get(memo.analyst_id)
            updated_memos.append(replace(memo, **updates) if updates else memo)

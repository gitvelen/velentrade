...
class DecisionService:
    ...
    def validate_cio_memo(self, packet: DecisionPacket, memo: CIODecisionMemo, optimizer_target_weights: dict[str, float]) -> DecisionGuardResult:
        ...
        execution_core_blocked = packet.execution_feasibility.get("execution_core_status") != "pass"
        reason_codes: list[str] = []
        if low_action:
            reason_codes.append("low_action_conviction_no_execution")
        if data_quality_blockers:
            reason_codes.append("data_quality_guard")
        if execution_core_blocked:
            reason_codes.append("execution_core_blocked_no_execution")
        if major:
        ...
        candidate_ref = new_id("decision-exception") if major and memo.deviation_reason and not low_action and not data_quality_blockers and not execution_core_blocked else None
        reopen_ref = new_id("reopen-recommendation") if low_action or data_quality_blockers or execution_core_blocked or (major and not memo.deviation_reason) else None

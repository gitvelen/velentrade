...
- 本附件是 Design 阶段 Web 工作台、高审美浅色主题、主导航、全局命令层、治理下团队工作区、三层 multi-agent 可视化、任务和审批的实现契约。
...
V1 只做桌面 Web 工作台。一级主菜单只允许 `全景 / 投资 / 财务 / 知识 / 治理`；`团队` 不作为一级菜单，团队能力管理下沉为治理下的二级工作区。老板默认页面使用“团队”，不使用 `Agent/Agents/AgentRun/ContextSnapshot` 等技术词；专业术语只保留在配置详情或审计详情。其他 route 都是某个主菜单下的子页面、详情页或审计页，不得在主导航中单独出现。
...
| 治理 | `/governance/team` | 团队工作区 | `TeamReadModel` | 二级工作区 |
| 治理 | `/governance/team/:agentId` | 团队画像详情页 | `AgentProfileReadModel` | drill-down 详情 |
...
- Attention badge：待办、系统健康、incident、Risk blocked；待办数量合并 pending approval 与 actionable/manual task。全景系统卡必须由 `/api/devops/health` / `DevOpsHealthReadModel` 推导，展示真实降级、恢复待验证或无法判断的原因，不得使用本地 Owner fixture 直接宣称“系统降级”。
- 当前页面内容区。

层级与来源规则：

- Owner Decision View 的 attention、todo、incident 卡片必须带明确目标：投资 workflow 跳 `/investment/:workflowId`，系统健康跳 `/governance?panel=health`，待办跳 `/governance?panel=todos`；待办面板内的审批项跳 `/governance/approvals/:approvalId`，manual_todo 跳对应业务模块办理，例如 `/finance?todo=:taskId`。
...
- Trace/Debug 不出现在 Owner 首屏默认任务卡正文中；默认任务卡只显示“查看详情”。投资页默认不放“审计”按钮，也不放 S0 审计回链卡；审计统一从治理审计模块或治理健康追溯进入。
...
- 默认页文案必须说人话：用“为什么影响决策”替代技术名词，用“现在不能继续，因为...”替代内部状态码，用“还缺哪些资料、补完后谁复核”替代 lineage/artifact/ref。默认页禁止出现 `AgentRun`、`ContextSnapshot`、`DecisionPacket`、`trace_id`、`artifact-*`、`ctx-*`、`reason_code` 等内部词；如必须展示专业术语，必须同时给出老板可读解释。
- 默认页信息密度按统一规则处理：1-2 块内容纵向或双列直接展开并把话说完整；3 块以上才使用摘要卡；矩阵和表格必须撑满有效宽度，不在宽屏留下大面积空白。全景、投资、财务、知识、治理、团队、审批和 Trace 都按同一规则排查。
...
- 卡片排列按统一信息架构执行：优先处理事项在前，核心状态和关键决策在中，支撑证据、健康和审计在后；治理页使用二级模块切换条暴露待办、团队、变更、健康和审计。
...
- confirmation 只确认 Request Brief；approval 只发生在审批模块。
...
| 区块 | 展示字段 | 空状态 | 错误状态 |
|---|---|---|---|
| Today Attention Strip | risk_items、todo_items、data_quality_items、incident_items | “当前无待处理事项” | 显示 last_success_at 和 retry |
...
| Todo Summary | pending approvals、manual_todo、到期时间、影响范围、风险提示 | 无待办 | 审批或待办列表失败时显示 stale/retry，不拆成两个 Owner 默认卡片 |
...
中：当前阶段老板需要看的判断、依据、风险、下一步
右/侧栏：仅在有足够 drill-down 内容时展示详情；1-2 个信息块默认直接展开
底部：必要时展示 Handoff、Attribution、Reflection linkback
```

必须组件：

- `StageRail`：显示每阶段业务状态、阶段名和阻断/完成情况；Owner 默认层不展示 `reason_code`、机器 ID 或 artifact 数。
...
- S0 只说明任务目标、边界、决策问题，以及是否已承诺不预设结论；不展示审计回链。
- S1 直接说明用了哪些数据、来自哪里、支持什么判断、还缺什么、影响哪个阶段。数据可用性不能只写抽象状态。
- S2 可用矩阵展示四位分析师方向、置信、证据、产物和硬异议；点击分析师才打开详情。
- S3 默认直接展示后端结构化辩论读模型中的老板可读结论、当前状态、核心分歧、观点变化、保留异议、CIO 综合和下一步；不能只给“辩论受阻”摘要，也不能由前端从零散字符串猜测业务详情。
- S4 默认直接展示决策依据；若没有形成决策，必须说清“为什么还不能决策、缺什么、补完后谁复核”。
- S5 只展示风控结论、风险原因和修复路径；不展示“审批与交易边界”这类说明性卡片。
- S6 只展示是否能成交、为什么不能成交、补齐什么后再看；不展示“审批边界”这类说明性卡片。
- S7 把归因状态合并到回链与反思入口，说明何时会产生复盘；不单独放“归因状态”卡。

S3/S6 解释边界：

- 当前 workflow 卡在 S3 时，页面必须优先说明 S3 阻断原因，例如“硬异议保留，需先补证并交风控复核”。
- `execution_core_status=blocked` 或 `execution_core_blocked_no_trade` 属于 S6 成交门槛；若同时展示，必须明确“执行数据不足是 S6 成交门槛，不是当前 S3 阻断原因”，不得把 S6 禁用成交原因写成 S3 受阻原因。

StageRail 交互：

- 每个 stage chip 可点击，点击后只改变 Dossier 的 `selected_stage` 和 URL query，不推进 workflow。
- 中间面板按 `selected_stage` 展示该阶段核心业务判断：S1 展示数据来源与缺口，S2 展示分析师矩阵，S3 展示四位分析师和 CIO 综合，S4 展示决策依据或不能决策原因，S5 展示风控结论与修复路径，S6 展示能否成交与补齐条件，S7 展示回链与反思入口。
- S3 点击详情必须按条目展开结构化后端字段：核心分歧详情展示为什么影响 S4、涉及岗位、当前结论和补证要求；观点变化详情按岗位展示变化前后、原因和影响；保留异议详情展示异议来源、反证风险、处置方式和禁止动作；轮次详情展示每轮议题、输入证据、处理结果和未解决问题。若结构化字段缺失，前端显示“后端未返回辩论详情”，不得编造。
- 侧栏只在 3 块以上或确有 drill-down 价值时展示相关数据、风险、审批、执行或归因详情；1-2 块内容不得强制打开抽屉。
- blocked/waiting stage 必须在 chip 和默认内容区同时显示老板能懂的业务原因；机器 `reason_code`、evidence refs 和 raw transcript 只进入审计或折叠详情。
...
治理页由五个治理模块组成，顶部必须展示二级模块切换条。团队模块必须直接展示各角色卡片，不能先展示“进入 Agent 团队”这类中转卡。二级模块的显示文案固定为 `待办 / 团队 / 变更 / 健康 / 审计`：

- 待办：Owner-facing 合并展示可处理审批、待补充、待办理、受阻或等待老板处理的任务；运行中、完成、展示型任务不出现在默认待办页；底层 API/read model 仍保持 `/api/tasks` 与 `/api/approvals` 分离。
- 团队：治理下二级工作区，查看九个角色的胜任度、CFO 归因、短板和能力提升入口；默认不展示 `Agent` 技术词。
- 变更：提示词、能力包、团队能力、配置、数据源策略、风险预算和执行参数 diff、生效边界、回滚；默认不展示机器 ID。
- 健康：数据源、数据质量、冲突、降级、运行事件；默认不展示 incident/recovery/check 原始 ID。
- 审计：以业务标题展示追溯记录；机器 ID 只允许在折叠详情或审计追踪层出现。

治理页默认排序：

1. 顶部摘要：待办、团队能力提升、服务健康。
2. 第一组：待办，先回答“现在谁要处理、能从哪里办理”。
3. 第二组：团队和变更，回答“系统能力怎么受控变化”。
4. 第三组：健康和审计，回答“运行是否可信、如何追溯”。

任务卡必须回答“谁接住了、现在卡在哪、下一步是什么”：
...
- `approval` 待办：显示业务标题、建议动作、影响范围和到期时间，动作进入真实审批详情并以后端 allowed actions 为准。
- `manual_todo` 待办：显示人工动作、原因、due_date、风险提示，并明确不连接审批/执行/交易链；没有通用完成 API 时只提供对应业务模块办理入口。
...
团队工作区回答“每个角色是否胜任岗位、短板在哪里，以及能力怎么受控调整”。它位于治理主菜单下，是老板管理 AI 团队的页面，不是开发者配置表。进入方式以治理页二级模块切换条为主，治理页团队卡片、全景团队能力卡和变更记录为辅助入口。团队工作区负责查看角色、绩效、短板和编辑方案；治理页的变更/审批模块负责方案状态、验证、高影响审批和生效边界。
...
- 高影响草案跳转审批模块，展示 diff、影响范围、替代方案、回滚方式和超时不生效。
...
- 团队工作区可编辑方案和查看上下文；变更/审批模块追踪方案状态、自动验证、高影响审批和生效边界。

### 7.4 团队禁用入口

- 不显示“立即更新在途 AgentRun”。
- 不显示绕过 Governance Change 的 Prompt/Skill/工具权限保存按钮。
- 不允许 Owner 在团队工作区直接修改 workflow state、Risk verdict、ApprovalRecord 或 PaperExecution。
...
- 默认卡片为每日简报、研究资料包、经验库、经验记录、资料关系、整理建议和上下文使用记录；不得使用“记忆工作区”“捕获记忆”“组织建议”“关系图”等 Owner 难以理解的标题。
- 每日简报必须用自然业务句表达影响和下一步，例如“半导体链关注度上升，先作为研究线索处理”，不得写成“今日影响：/下一步：”模板句。
- 研究资料包必须在默认视图直接展示来源类别、复核状态和证据数量；不得把“资料包在哪里、从哪里来”藏在折叠区。
- 经验记录用于保存一条可复核经验；按钮文案为“保存经验”，成功后只说明“可在经验记录里复核”，不得暴露 Memory ID、Gateway 写入 ID 或版本号。
- 资料关系只展示契约关系的 Owner-facing 摘要和样例；关系为空时说明“暂无已确认关系，应用整理建议后再生成资料关系”。
- 整理建议用于把经验记录归类到资料包和标签，Owner/Researcher 确认后仍走 Gateway；按钮文案为“应用整理建议”，前端不得直接覆盖旧 MemoryVersion，也不得把建议说成已改变业务事实或默认上下文。
- Context injection inspector：在 Knowledge 或 Trace 中转译为上下文使用记录；默认不展示 `why_included`、raw process archive、MemoryVersion、relation id 或 denied refs。
- Knowledge/Prompt/Skill Proposal 不在知识页办理；相关提案进入治理的变更/审批模块。
- 知识页不直接使提案或 DefaultContext 生效。

## 9. 加载、空、错状态

- 页面级 loading 使用 skeleton，不改变当前已加载业务状态。
- read model 加载失败显示错误卡片和重试按钮；trace_id 只在折叠详情或审计层展示。
...
- 治理下团队工作区存在九个角色卡、画像详情、胜任度评分、CFO 归因引用、能力短板和能力提升入口；默认列表不展示版本、上下文快照或机器 ID。

`governance_task_report.json` 必须断言：

- 待办中合并展示可处理 approval 与 actionable/manual task。
- API/read model 层仍保持 approval_center 只包含真正 approval，task_center 只包含 Task Envelope 投影。
...
- TeamReadModel 覆盖治理下团队工作区的九个正式角色。
...
|---|---|---|
| Shell / Navigation | 五个一级主菜单、治理下团队工作区、route canonical parent、来源入口和返回路径是否清晰 | command triage、attention badge |
| Overview | Owner 今天要不要处理；风险、待办、incident 是否需要动作 | happy path、Risk rejected、incident |
...
| 治理团队 | 九个角色是否胜任岗位；能力配置如何受控进入治理 | agent capability matrix、prompt/skill/context versions、draft governance |
| Governance / Approval | 待办、变更、健康、审批包 | high impact governance、Owner timeout |
...
中部：当前阶段老板可读结论
  - S1：数据来源、用途、缺口和阶段影响
  - S2：Analyst Stance Matrix
  - S3：四位分析师观点、依据、反证、结论变化和 CIO 综合
  - S4：决策依据或不能决策原因
  - S5/S6/S7：风控、成交、回链反思的直接结论
侧栏：只有真正需要 drill-down 时展示详情；1-2 块信息直接展开
底部：必要时展示 Handoff / Evidence Map / Attribution / Reflection
```
...
- 治理下团队工作区的能力配置保存只生成治理变更草案，不直接更新有效版本或在途 AgentRun。
...
- 主导航、中文高端浅色卡面和三层视图断言。
- 治理下团队工作区 Markdown 预览。
- Request Brief preview flow。
- 禁用入口 UI/read model/API guard 对齐。

`governance_task_report.json` 必须引用：

- 待办合并 UI 与 Task/Approval/manual_todo 契约分离 Markdown 预览或 read model snapshot。
